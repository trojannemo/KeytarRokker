...
namespace KeytarRokker
{
    class MIDIStuff
    {
        ...
        private const int KEYS_SOLO = 103;
        ...
        public bool ReadMIDIFile(string midi)
        {
            ...
            try
            {
                ...
                for (var i = 0; i < MIDIFile.Events.Tracks; i++)
                {
                    ...
                    else if (trackname.Contains("PART KEYS"))
                    {
                        List<MIDINote> toadd;

                        MIDI_Chart.KeysX.Overdrive = GetSpecialMarker(MIDIFile.Events[i], KEYS_OD);
                        CheckMIDITrack(MIDIFile.Events[i], MIDI_Chart.KeysX.ValidNotes, out toadd);
                        MIDI_Chart.KeysX.ChartedNotes.AddRange(toadd);
                        MIDI_Chart.KeysX.Solos = GetSpecialMarker(MIDIFile.Events[i], KEYS_SOLO);

                        MIDI_Chart.KeysH.Overdrive = MIDI_Chart.KeysX.Overdrive;
                        CheckMIDITrack(MIDIFile.Events[i], MIDI_Chart.KeysH.ValidNotes, out toadd);
                        MIDI_Chart.KeysH.ChartedNotes.AddRange(toadd);
                        MIDI_Chart.KeysH.Solos = MIDI_Chart.KeysX.Solos;

                        MIDI_Chart.KeysM.Overdrive = MIDI_Chart.KeysX.Overdrive;
                        CheckMIDITrack(MIDIFile.Events[i], MIDI_Chart.KeysM.ValidNotes, out toadd);
                        MIDI_Chart.KeysM.ChartedNotes.AddRange(toadd);
                        MIDI_Chart.KeysM.Solos = MIDI_Chart.KeysX.Solos;

                        MIDI_Chart.KeysE.Overdrive = MIDI_Chart.KeysX.Overdrive;
                        CheckMIDITrack(MIDIFile.Events[i], MIDI_Chart.KeysE.ValidNotes, out toadd);
                        MIDI_Chart.KeysE.ChartedNotes.AddRange(toadd);
                        MIDI_Chart.KeysE.Solos = MIDI_Chart.KeysX.Solos;
                    }
            ...
            MIDI_Chart.KeysX.Sort();
            MIDI_Chart.KeysH.Sort();
            MIDI_Chart.KeysM.Sort();
            MIDI_Chart.KeysE.Sort();
    ...
    {
        public MIDITrack KeysX { get; set; }
        public MIDITrack KeysH { get; set; }
        public MIDITrack KeysM { get; set; }
        public MIDITrack KeysE { get; set; }
        ...
        public void Initialize()
        {
            KeysX = new MIDITrack { Name = "Keys X", ValidNotes = new List<int> { 100, 99, 98, 97, 96 } };
            KeysH = new MIDITrack { Name = "Keys H", ValidNotes = new List<int> { 88, 87, 86, 85, 84 } };
            KeysM = new MIDITrack { Name = "Keys M", ValidNotes = new List<int> { 75, 74, 73, 72 } };
            KeysE = new MIDITrack { Name = "Keys E", ValidNotes = new List<int> { 62, 61, 60 } };
